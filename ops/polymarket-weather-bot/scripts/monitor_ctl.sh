...
cmd_start() {
  ...
  nohup bash -lc "
    ...
    while true; do
      ...
      '$PYTHON_BIN' '$ROOT_DIR/paper_runner.py' --apply --min-hours-to-expiry '$MIN_HOURS_TO_EXPIRY' >> '$LOG_FILE' 2>&1 || true
      sleep '$INTERVAL_SEC'
    done
  " >/dev/null 2>&1 &
...
cmd_status() {
  if is_running; then
    echo "monitor: running (pid=$(cat "$PID_FILE"), interval=${INTERVAL_SEC}s, min_hours_to_expiry=${MIN_HOURS_TO_EXPIRY})"
  else
...
cmd_run_once() {
  cd "$ROOT_DIR"
  "$PYTHON_BIN" "$ROOT_DIR/paper_runner.py" --apply --min-hours-to-expiry "$MIN_HOURS_TO_EXPIRY"
}
