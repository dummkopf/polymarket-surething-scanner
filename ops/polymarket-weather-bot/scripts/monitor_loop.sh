...
MAX_COORD_DATES_PER_TICK="${MAX_COORD_DATES_PER_TICK:-30}"
EXTERNAL_NO_CONSENSUS_GATE_ENABLED="${EXTERNAL_NO_CONSENSUS_GATE_ENABLED:-1}"
EXTERNAL_NO_CONSENSUS_MARGIN_C="${EXTERNAL_NO_CONSENSUS_MARGIN_C:-0.0}"
...
while true; do
  ...
  timeout "${RUN_TIMEOUT_SEC}s" "$PYTHON_BIN" "$ROOT_DIR/paper_runner.py" --apply \
    ...
    --max-coord-dates-per-tick "$MAX_COORD_DATES_PER_TICK" \
    --external-no-consensus-gate-enabled "$EXTERNAL_NO_CONSENSUS_GATE_ENABLED" \
    --external-no-consensus-margin-c "$EXTERNAL_NO_CONSENSUS_MARGIN_C" \
