...
## 十、新增策略：事件冲击“跳变-滞后传播”模式（你提到的图形）

### 10.1 模式定义（不是简单追涨）
你给的图里，核心不是“涨到 90% 就买”，而是三段结构：
1. 前期有小幅偏离（modest gap）
2. 某一腿突然跳变（例如 50%→90%）
3. 相关腿/相关市场存在短暂“未完全反映”的滞后

**真正可交易的是第3段（传播滞后），不是第2段（表面大涨）。**

### 10.2 为什么可能有 edge
- 信息到达不是连续扩散，而是“先冲一个盘口，再扩散到关联盘口”。
- 许多参与者只盯单一市场，较少做事件内一致性约束（binary 对偶、neg risk 转换关系）。
- 跳变时库存和风控约束会让做市商短时失衡，给出可编程的短窗错价。
...
**B. Exhaustion-Fade（次级）**
- 仅在“无外部确认 + 订单簿失衡回落”时做反转；
- 风险更高，优先级低于 A。
...
- `J` = 领先腿短时跳变幅度（如 90 秒内）
- `P` = 跳后价格
- `L` = 一致性偏离（如 binary 的 `|pA + pB - 1|` 或事件约束残差）
- `V` = 成交量突增倍数（相对近 30 分钟基线）
...
执行：
- 优先做 **滞后腿**（maker 优先，必要时小额 taker 修正）
- 不在 `P > 0.92` 直接追领先腿（除非有强外部确认）
...
### 10.5 失败场景（必须防）
- 真正新信息是“范式切换”，不是短期错价（你会把趋势当噪音）。
- 盘口太薄，多腿执行失败，裸露方向风险。
- 临近结算时微结构极端，tick 变化放大账面波动。

### 10.6 对该想法的判断
**结论：值得做，而且应纳入主研究路线。**
但定位应是：
- ✅ “跳变后的滞后传播”策略（可结构化、可自动化）
- ❌ “看到 >90% 就追”策略（容易被反向选择）

建议把它列为独立模块：`S4 Shock-Lag Propagation`，与做市主线并行评估。
